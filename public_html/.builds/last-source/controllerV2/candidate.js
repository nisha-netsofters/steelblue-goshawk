...
exports.createCandidates = async (req, res) => {
  ...
  try {
    ...
    if (professional) professional = JSON.parse(req.body.professional);
    const industriesId = [];
    industries_relation = JSON.parse(req.body.industries_relation);
    ...
    const jobCategories = await JobCategory.findOne({ id: jobCategoryId });
    ...
    const candidate = await Candidates.create({
      ...
      // welcome email (optional, queued)
      // await enqueueEmailJob("candidateWelcome", {
      //   candidate: responce,
...
exports.getCandidates = async (req, res) => {
  ...
  try {
    ...
    // console.log("filterForProfessional", filterForProfessional);
    // console.log("jobCategoriesDiv", jobCategoriesDiv);
    // console.log("preferedJobLocation", preferedJobLocation);
    // console.log("filter", filter);
    // console.log("industriesIdDiv", industriesIdDiv);
            ...
            $switch: {
              branches: [
                ...
                },
                {
                  ...
                  }, // Check if difference is more than 15 days
                  then: "view",
                },
                {
              ...
              default: "new", // Default to "new" if neither condition is met
            },
          },
        },
      },
    ...
    const result = {
      data: candidate,
      count: pipeline.some(stage => stage.$match && Object.keys(stage.$match).length > 0) ? candidate.length : totalCandidates,
    };
    res.json({
      results: result.data,
      total: result.count,
...
exports.checkCandidate = async (req, res) => {
  ...
  try {
    let msg = {};
    const mobileData = await Candidates.findOne({ mobile: mobile });
    // .where("mobile", mobile);
    const emailData = await Candidates.findOne({ email: email });
    if (emailData) {
      res.json({
