...
const getAssetBaseUrl = () => {
  const raw =
    ...
    "";
  // Prefer API host for /uploads — PORT default 7001
...
const resolvePlaceholders = (text, candidate) => {
  ...
  Object.entries(PLACEHOLDER_MAP).forEach(([key, fn]) => {
    ...
    resolved = resolved.replace(new RegExp(key.replace(/[{}]/g, "\\$&"), "gi"), replacement);
  });
  return resolved;
};
...
const parseParamValue = (raw, candidate) => {
  if (raw == null) return "";
  const asString = resolvePlaceholders(String(raw), candidate);
...
const sendSingleApi = async (api, candidate) => {
  ...
  try {
    ...
    let sanitized = sanitizeTemplatePayload(payload, { parameterMode });
