...
const resolvePlaceholders = (text, candidate) => {
  ...
  // Bare token saved from UI (e.g. "firstname") without braces
  if (/^[a-z0-9_]+$/i.test(resolved) && PLACEHOLDER_MAP[`{{${resolved.toLowerCase()}}}`]) {
    return resolveByToken(resolved, candidate);
  }
    ...
    if (compType === "body" || compType === "button" || compType === "footer") {
      ...
      (comp.parameters || []).forEach((p) => {
        ...
        if (p.type === "text") {
          ...
          // Still an unresolved {{placeholder}} → try resolve again / fall back to parameter_name
          if (/^\{\{[^}]+\}\}$/.test(text)) {
          ...
          // Named WhatsApp templates often use parameter_name = firstname / lastname
          if (!text && paramName) {
