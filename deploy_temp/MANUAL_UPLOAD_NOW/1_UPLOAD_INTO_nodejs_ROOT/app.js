...
// Always register check routes first — proves THIS app.js is live after deploy
app.get("/api/candidate/toggle-favorite-check", (req, res) => {
  res.json({
    status: "ok",
    route: "/api/candidate/toggle-favorite",
    method: "POST",
    appFile: "nodejs/app.js",
    time: new Date().toISOString(),
  });
});
app.get("/api/candidate/toggle-favorite", (req, res) => {
  res.json({
    status: "ok",
    message:
      "toggle-favorite route is live. Use POST from the app (star click).",
    method: "POST",
    time: new Date().toISOString(),
  });
});

// Hard-register favorite toggle so Hostinger deploys never miss this route
try {
  ...
  console.error("Failed to register toggle-favorite POST route:", err.message);
  app.post("/api/candidate/toggle-favorite", (req, res) => {
    res.status(500).json({
      msg: "toggle-favorite handler failed to load",
      error: err.message,
    });
  });
}
