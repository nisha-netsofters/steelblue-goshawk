...
function buildAiError(provider, status, apiMsg = "") {
  ...
  // Check quota/billing FIRST — Google often returns 429 with free_tier limit: 0
  // (new AQ. auth keys on Free tier without billing). Do not call this "invalid key".
  const isQuota =
    status === 429 ||
  ...
  if (status === 404 || msg.includes("not found") || msg.includes("is not found") || msg.includes("no longer available")) {
    ...
    err.message =
      "Invalid AI Model. Please set a valid Model (recommended: gemini-2.0-flash or gemini-flash-latest) in Super Admin → OCR & API Configuration.";
    return err;
  }
  ...
  if (status === 503 || msg.includes("high demand") || msg.includes("unavailable")) {
    ...
    err.message =
      "Gemini model is temporarily unavailable (high demand). Please wait a moment and try again.";
    return err;
  }
...
async function queryGemini(text, credentials) {
  ...
  const modelsToTry = [
    ...new Set([preferred, ...fallbackModels].filter(Boolean)),
  ];
  const prompt = getAiPrompt(text);
  const errors = [];
  ...
  for (const activeModel of modelsToTry) {
    try {
      // Prefer header auth (works for both AIza and new AQ. auth keys)
      ...
      // Try next model when deprecated/missing (404), rate-limited (429), busy (503), or bad request for that model
      if (
        status === 404 ||
        status === 429 ||
        status === 400 ||
  ...
  const fail = new Error(
    `All Gemini models failed. Update Model in OCR & API Configuration (recommended: gemini-2.0-flash or gemini-flash-latest). Details: ${joined.slice(0, 300)}`
  );
  fail.code = "AI_PARSE_FAILED";
  throw fail;
}
